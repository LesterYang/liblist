...
{
    lst_assert(list);
    lst_check(list, (extetype)NULL_LIST);
...
{
    lst_assert(list);
    lst_check(list, (sorttype)NULL_LIST);
    return list->sort;
}
...
{
    lst_assert(list);
    lst_check(list, NULL_LIST);

	if (LST_UNLIKELY(list->init)) return INIT_NOT_DONE;
...
{
    lst_check(list, (const char*)NULL_LIST);
